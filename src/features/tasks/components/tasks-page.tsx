  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTask) {
        ...
        if (!updateError) {
          await supabase.from('activities').insert({
            action: `Updated task "${form.title.trim()}"`,
            entity_type: 'task',
        ...
        if (!insertError) {
          await supabase.from('activities').insert({
            action: `Created task "${form.title.trim()}"`,
            entity_type: 'task',
            user_id: currentUser?.id,
          })
        }
      ...
      {!loading && !error && tasks.length > 0 && filteredTasks.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            No {activeTab === 'todo' ? 'to-do' : activeTab === 'in_progress' ? 'in-progress' : 'completed'} tasks
          </p>
          <Button
            variant="outline"
            size="sm"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge
                          variant="outline"
                          className={`text-[10px] font-medium capitalize border-transparent ${PRIORITY_BADGE[priority]}`}
                        >
                          {priority}
                        </Badge>
                        {task.deadline && (
